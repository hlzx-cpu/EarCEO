...
class OpenOPCAdapter(HarnessAdapter):
    ...
    async def spawn(self, task: str, *, repo_path: str, context: dict | None = None) -> str:
        ...
        runner = asyncio.create_task(self._run(handle, cmd))
        _handles[handle]["_task"] = runner
        return handle
    ...
    async def _run(self, handle: str, cmd: list[str]) -> None:
        ...
        try:
                ...
                async for raw_line in proc.stdout:
                    ...
                    if typ in ("message", "final"):
                        ...
                        _handles[handle]["events"].append(
                            StatusEvent(
                                kind="error",
                                text=payload.get("error", str(payload)),
                            ),
                        )
                        break
            ...
            proc = _handles[handle].get("_proc")
            if proc is not None and proc.returncode is None:
                proc.kill()
                await proc.wait()
            _handles[handle]["status"] = "cancelled"
            raise
        except asyncio.TimeoutError:
            proc = _handles[handle].get("_proc")
            if proc is not None and proc.returncode is None:
            ...
            if _handles[handle]["status"] != "cancelled":
    ...
    async def result(self, handle: str) -> TaskResult:
        ...
        while store["status"] not in ("done", "failed", "cancelled"):
            await asyncio.sleep(0.05)
    ...
    async def cancel(self, handle: str) -> None:
        ...
        if store and store["status"] in ("pending", "running"):
            ...
            proc = store.get("_proc")
            if proc is not None and proc.returncode is None:
                proc.kill()
                await proc.wait()
            runner = store.get("_task")
