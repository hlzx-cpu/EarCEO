...
## D11: Durable task lifecycle and one canonical task ID
- **Context:** async dispatch swallowed exceptions, returned no result, and lost
  live tasks because Web created a new Receptionist per request. Web and core
  also generated different IDs, while MCP performed an independent unsafe
  read/write cycle on the same JSON file.
- **Decision:** persist every task under one canonical `task_id` with
  `pending/running/completed/failed/cancelled`; use one execution core for sync
  and async dispatch; store results and structured errors; make Web
  process-scoped; route Web, MCP, and Receptionist through `JsonStateStore`.
- **Storage:** thread lock + `flock` + atomic `os.replace`; all compound changes
  use one locked `update()` transaction. Corrupt JSON is surfaced, not reset.
- **Cancellation boundary:** a process may cancel only tasks whose live adapter
  it owns. Persisted remote tasks remain inspectable but are not falsely marked
  cancelled.
